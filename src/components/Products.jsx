...
const Products = () => {
  ...
  const productsItems = data?.map((pro) => (
    <div className="p-3 shadow-lg bg-white" key={pro.id}>
      <img
        ...
        alt=""
      />
      <h3 className="text-blue-700 font-semibold">{pro.title}</h3>
      <p className="text-gray-800">${pro.price}-USD</p>
  ...
  return (
    <div className="container max-w-[1250px] m-auto">
      {loading && <Skeleton />}
      <div className="grid grid-cols-5 gap-3 p-3 mt-10 max-lg:grid-cols-3 max-sm:grid-cols-2 max-[450px]:grid-cols-1">
        {productsItems}
      </div>
      {modalData && (
        <Modal close={() => setModalData(null)}>
          <div className="bg-white flex gap-6 p-3 rounded-xl items-center content-center">
            ...
            <div className="w-[50%]">
              ...
              <p className="text-base text-blue-900 py-5">
